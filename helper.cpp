...
std::vector<Boid> generateBoids(int numBoids, double xMin, double xMax, double yMin, double yMax, 
...
//I've avoided calling every time vector.size() in for (int i ...) so used iterators
...
std::pair<double, double> averageDistanceAndStdDev(std::vector<Boid> const& boids) {
    ...
    double variance = (count > 1) ? ((totalDistanceSquared / count) - (averageDistance * averageDistance)) : 0.0;
    double stdDev = std::sqrt(variance);

    return {averageDistance, stdDev};
}

std::pair<std::pair<double, double>, std::pair<double, double>> averageVelocityAndStdDev(std::vector<Boid> const& boids) {
    double totalVx = 0.0;
    double totalVy = 0.0;
    ...
    double varianceVx = (count > 1) ? ((totalVxSquared / count) - (avgVx * avgVx)) : 0.0;
    double varianceVy = (count > 1) ? ((totalVySquared / count) - (avgVy * avgVy)) : 0.0;
